		...
		if ( !this._mkdirp ) {
			...
			this._mkdirp = new ResourceManager( function( path, cb ) {
				...
				function onRemote( err, remote ) {
					...
					if ( err ) return final( err );
					if ( remote && !remote.ftp.type.match( /^dir/i ) ) return final( new Error( path + ' is a file, cannot MKDIR' ) );
		...
		if ( !this._list ) {
			...
			this._list = new ResourceManager( function( path, cb ) {
				...
				function onFiles( err, files ) {
					...
					// no such file or directory
					if ( err && err.code === 550 ) return final( null, [] );
					if ( err ) return final( err );
	...
	ftp: function( cb ) {
		...
		if ( this.used.length < this.config.maxConnections ) {
			...
			ftp.on( 'error', function( err ) {
				...
				self.log( 'ERROR', err.message + code );
